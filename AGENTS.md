...
5. **Causal edge signaling** – Marching‑ants pulses remain active on every reachable causal edge, including alternate paths into a node, when causal flow is on; they respect `flowPulseMs` and degrade gracefully to static straight edges when disabled. Anchor handles stay aligned so arrows attach to the closest cardinal face.
